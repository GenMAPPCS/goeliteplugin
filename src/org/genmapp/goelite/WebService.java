...
// The classes in edu.sdsc.nbcr.* were auto-generated by Axis based on the GOEliteService.wsdl.xml
// This WebService class is the user-written client code that interfaces directly with those classes
// The rest of the plugin can basically stay free from all that mess

// The order of operations for a simple request are
// 1) get a service object
// 2) fill the service object with input args
// 3) launch a job request ( via service.launchJob() )
// 4) fetch the output from the service

// Note, in my docs and code I use the terms server/service sort of interchangeably ( sloppily )
class WebService {	
	...
	// uses the AppServiceLocator to get a service object, which serves as the conduit
